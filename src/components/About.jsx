...
import { Target, Award, Users, TrendingUp, ShieldCheck, Globe } from 'lucide-react';
...
<div className="text-center mb-12">
  ...
  <p className="text-lg md:text-xl text-slate-400 max-w-3xl mx-auto leading-relaxed">
    EduTrack is a high‑performance ecosystem uniting theory with industry mastery — 
    empowering institutions and professionals to push boundaries and achieve more.
  </p>
</div>
    ...
    <div className="flex flex-col md:flex-row gap-8 items-center">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h3 className="text-2xl font-bold text-white tracking-tight">Our Strategic Philosophy</h3>
        </div>
        
        <p className="text-slate-300 text-base leading-relaxed mb-4">
          We believe that education must be as dynamic as the markets it serves. EduTrack integrates 
          the **rigorous standards** of traditional academia with the **high-speed agility** of 
          modern technological hubs. 
        </p>
        <p className="text-slate-400 text-base leading-relaxed">
          By facilitating a seamless exchange between global industry experts and academic researchers, 
          we create a fluid learning environment.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 flex-1 w-full">
        <div className="p-5 bg-slate-800/40 border border-slate-700/50 rounded-2xl hover:border-emerald-500/50 transition-colors">
          ...
          <div className="text-white font-bold text-sm">Accredited Content</div>
        </div>
        <div className="p-5 bg-slate-800/40 border border-slate-700/50 rounded-2xl hover:border-emerald-500/50 transition-colors">
...
<div className="mb-24">
  <div className="text-center mb-12">
    <h3 className="text-2xl md:text-3xl font-bold text-white tracking-tight">
      Engineering a <span className="text-emerald-400">Superior</span> Experience
    </h3>
  </div>
  ...
  <div className="grid md:grid-cols-3 gap-6">
    ...
    {[
      ...
      >
        ...
        <div className={`w-12 h-12 ${feature.bg} rounded-xl flex items-center justify-center mb-6 transition-transform group-hover:scale-110`}>
          {feature.icon}
        </div>
...
<div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
  {[
    ...
    <div key={i} className="group relative overflow-hidden bg-slate-900/40 border border-slate-800/50 p-6 md:p-8 rounded-2xl transition-all hover:bg-slate-900/80">
      {/* Decorative background glow on hover */}
      <div className={`absolute -inset-px bg-gradient-to-r ${stat.col} opacity-0 group-hover:opacity-10 transition-opacity duration-500`} />
      
      <div className="relative z-10">
        <div className={`text-3xl md:text-4xl font-bold bg-gradient-to-r ${stat.col} bg-clip-text text-transparent mb-1`}>
          {stat.val}
        </div>
        <div className="text-white font-semibold text-sm mb-1">{stat.suffix}</div>
      ...
      <div className={`absolute inset-x-0 bottom-0 h-0.5 bg-gradient-to-r ${stat.col} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left`} />
    </div>
  ))}
</div>
      </div>
