...
//                 {/* --- DATA TABLE CARD ---Data dynamically rendered using map function */}
...
//                                             <th className="py-4 text-slate-500 text-uppercase tracking-widest" style={{ fontSize: '11px' }}>Description</th>
...
//                                                 <td className="py-4">
//                                                     <div className="text-slate-400" style={{ fontSize: '13px', maxWidth: '250px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
...
//                 {/* --- STYLED MODAL -adding/updating qualifications with controlled i/ps-- */}
//                 <Modal 
//                     show={show} 
...
const QualificationManager = () => {
    ...
    const handleSave = async () => {
        ...
        if (!form.qualificationName || !form.qualificationsh || !form.qualificationYears || !form.qualificationDescription) {
            return alert("All fields are required. Please fill in everything before saving.");
        }

        try {
            if (isEdit) {
                // Ee call ippudu complete 'form' object ni backend ki pamputhundi (Updated name, years, etc.)
                await adminService.editQualification(oldName, form);
            } else {
                await adminService.addQualification(form);
            }
            ...
            console.error("Error saving data", err);
            const errorMessage = err.response?.data?.message || "Action failed. Check for duplicate entries.";
            alert(errorMessage);
        }
    };
        ...
        <div className="course-mgmt-dark min-vh-100">
            <Container className="py-5">
                <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-5 gap-3">
                    ...
                    <button className="btn-teal-action shadow-lg" onClick={handleAddNew}>
                        <FiPlus size={20} /> Create Qualification
                    </button>
                </div>
                ...
                <Modal show={show} onHide={() => setShow(false)} centered contentClassName="modal-content-dark border-0 shadow-2xl">
                    <Modal.Header closeButton closeVariant="white" className="border-0 px-4 pt-4">
                        <Modal.Title className="fw-bold text-white">
                            {isEdit ? 'Update' : 'Register'} <span className="text-teal">Qualification</span>
                        </Modal.Title>
            ...
            <style>{`
                ...
                .text-slate-400 { color: #94a3b8 !important; }
                .text-slate-500 { color: #475569 !important; }
                .btn-teal-action { background: #14b8a6; color: white; border: none; padding: 10px 24px; border-radius: 12px; font-weight: 600; display: flex; align-items: center; gap: 8px; transition: 0.3s; }
                .btn-teal-action:hover { transform: translateY(-2px); box-shadow: 0 10px 20px rgba(20, 184, 166, 0.2); }
                .selection-card-dark { background: #0f172a; border-radius: 20px; border: 1px solid #1e293b; }
                .selection-label-dark { font-size: 11px; font-weight: 800; color: #475569; text-transform: uppercase; margin-bottom: 8px; display: block; }
                .custom-table-dark thead tr { background-color: #0f172a; border-bottom: 2px solid #1e293b; }
                .custom-table-dark tbody tr:hover { background-color: #1e293b !important; }
                .border-slate-800 { border-color: #1e293b !important; }
                .pulse-icon-dark-static { width: 40px; height: 40px; background: rgba(20, 184, 166, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; }
                .btn-icon-dark { background: none; border: none; transition: 0.2s; padding: 5px; cursor: pointer; }
                .btn-icon-dark:hover { transform: scale(1.2); }
                .modal-content-dark { background-color: #0f172a !important; border-radius: 24px !important; }
                .form-select-dark { background-color: transparent; border: none; color: #f1f5f9; font-weight: 500; outline: none; width: 100%; }
                .form-select-dark:focus { background-color: #020617; border-color: #14b8a6 !important; box-shadow: none; color: white; }
            `}</style>
        </div>
    );
